...
ENT.JumpSound = Sound("npc_los_minions/jump.mp3")
...
local workshopID = "" -- set Steam Workshop ID after publish

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_los_minions_acquire_distance =
	CreateConVar("npc_los_minions_acquire_distance", 2000, FCVAR_NONE,
	"The maximum distance at which Los Minions will chase a target.")

local npc_los_minions_spawn_protect =
	CreateConVar("npc_los_minions_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, Los Minions will not target players or hide within 200 units of \z
	a spawn point.")

local npc_los_minions_attack_distance =
	CreateConVar("npc_los_minions_attack_distance", 80, FCVAR_NONE,
	"The reach of Los Minions' attack.")

local npc_los_minions_attack_interval =
	CreateConVar("npc_los_minions_attack_interval", 0.2, FCVAR_NONE,
	"The delay between Los Minions' attacks.")

local npc_los_minions_attack_force =
	CreateConVar("npc_los_minions_attack_force", 500, FCVAR_NONE,
	"The physical force of Los Minions' attack. Higher values throw things \z
	farther.")

local npc_los_minions_smash_props =
	CreateConVar("npc_los_minions_smash_props", 1, FCVAR_NONE,
	"If set to 1, Los Minions will punch through any props placed in their way.")

local npc_los_minions_allow_jump =
	CreateConVar("npc_los_minions_allow_jump", 1, FCVAR_NONE,
	"If set to 1, Los Minions will be able to jump.")

local npc_los_minions_hiding_scan_interval =
	CreateConVar("npc_los_minions_hiding_scan_interval", 3, FCVAR_NONE,
	"Los Minions will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant Los Minions are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_los_minions_hiding_repath_interval =
	CreateConVar("npc_los_minions_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to Los Minions' hiding spot will be redetermined every X seconds.")

local npc_los_minions_chase_repath_interval =
	CreateConVar("npc_los_minions_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of Los Minions' target will be redetermined every \z
	X seconds.")

local npc_los_minions_expensive_scan_interval =
	CreateConVar("npc_los_minions_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_los_minions_force_download =
	CreateConVar("npc_los_minions_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download Los Minions resources \z
...
if npc_los_minions_force_download:GetBool() and workshopID ~= "" then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("los_minions_nag")
util.AddNetworkString("los_minions_navgen")
...
local VECTOR_los_minions_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_los_minions_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_los_minions: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other Los Minions, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_los_minions"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "los_minionsMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_los_minions" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why Los Minions isn't working.
	net.Start("los_minions_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_los_minions: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_los_minions: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_los_minions: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_los_minions: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_los_minions: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, Los Minions in pairs(ents.FindByClass("npc_los_minions")) do
		los_minions:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "los_minionsNavGen", navEndGenerate)
	else
		print("npc_los_minions: nav_generate failed to initialize")
...
concommand.Add("npc_los_minions_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_los_minions: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. Los Minions doesn't use
	...
	elseif isConsole then
		print("npc_los_minions: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_los_minions_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_los_minions_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_los_minions_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_los_minions_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. Los Minions means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_los_minions_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_los_minions_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_los_minions_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_los_minions_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_los_minions_attack_distance:GetInt()
		...
		local repathInterval = npc_los_minions_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_los_minions_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_los_minions_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_los_minions_hiding_repath_interval:GetFloat()
...
local MAT_los_minions = Material("npc_los_minions/los_minions")
killicon.Add("npc_los_minions", "npc_los_minions/killicon", color_white)
language.Add("npc_los_minions", "Los Minions")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_los_minions: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a Los Minions.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_los_minions_music_volume =
	CreateConVar("npc_los_minions_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by Los Minions. (0-1, where 0 is muted)")

-- If another Los Minions comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, Los Minions do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 2000000

-- Max volume is achieved when MUSIC_los_minions_PANIC_COUNT Los Minions are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of Los Minions.
local MUSIC_los_minions_PANIC_COUNT = 1

local MUSIC_los_minions_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_los_minions_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_los_minions") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("los_minionsPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_los_minions_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_los_minions" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_los_minions_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear Los Minions taunt us.
...
local function startTimer()
	if not timer.Exists("los_minionsPanicMusicUpdate") then
		timer.Create("los_minionsPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_los_minions)

	-- Get the normal vector from Los Minions to the player's eyes, and then compute
	...
	-- Los Minions should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("los_minionsHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("los_minionsHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("los_minions is studying this map.", "los_minionsHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "los_minionsHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "los_minionsHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "los_minionsHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "los_minionsHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("los_minions_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "los_minionsNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. Los Minions doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_los_minions_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for Los Minions to figure \z
		this map out.\n\z
...
net.Receive("los_minions_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! Los Minions doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet Los Minions-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! Los Minions doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to Los Minions.\n\z
			\n\z
			If you ARE the server host, you can run npc_los_minions_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not Los Minions-compatible!",
			"Ok", nil,
